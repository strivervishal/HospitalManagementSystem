...
const Appointment = () => {
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <button className="md:hidden p-4 text-gray-700" onClick={() => setSidebarOpen(!sidebarOpen)}>
        <FaBars className="text-2xl" />
      </button>

      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 w-64 bg-white shadow-md transform ${
  ...
  <thead>
    <tr className="bg-[#e8f8f8] text-left">
      {["Patient Name", "Assigned Doctor", "Date", "Time", "Action"].map((header, index) => (
        <th key={index} className="p-3 text-gray-600 border-b border-gray-300 text-[13px] md:text-[15px]">
          {header}
        </th>
      ))}
    </tr>
  ...
  <tbody>
    {appointments.map((appointment) => (
      <tr key={appointment._id} className="border-b border-gray-300 hover:bg-gray-100 transition">
        <td className="p-3 text-[13px] md:text-[15px]">{appointment.patientName}</td>
        <td className="p-3 text-[13px] md:text-[15px]">{appointment.assignedDoctor}</td>
        <td className="p-3 text-[13px] md:text-[15px]">{appointment.date}</td>
        <td className="p-3 text-[13px] md:text-[15px]">{appointment.time}</td>
        <td className="p-3 flex space-x-3">
          <button onClick={() => handleEdit(appointment)} className="text-blue-500 hover:text-blue-700">
